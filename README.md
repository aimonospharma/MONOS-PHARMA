...
> Production дээр `SECRET_KEY`, `ADMIN_EMAILS` дутуу бол апп **зориудаар эхлэхгүй**
> — чимээгүй эмзэг байдалтай ажиллахаас сэргийлсэн.
...
| **Нууц үг** | SSO залгаагүй үед | Мэйл + нууц үг (нэр асуухгүй) |
| **Demo** | `DEMO_MODE=1` (зөвхөн локал) | Demo account-ууд, нууц үггүй |

`AZURE_*` гурвыг бөглөмөгц **нууц үгийн форм өөрөө алга болно** — код өөрчлөх
шаардлагагүй.

#### Нууц үгийн горим — хэрхэн ажилладаг

| Дүрэм | |
|---|---|
| Домэйн | Зөвхөн `@monos.mn` (`ALLOWED_EMAIL_DOMAINS`) |
| Нууц үг | Мэйлийн `@`-аас өмнөх хэсэг. `ner.o@monos.mn` → `ner.o` |
| Хэн нэвтэрч чадах | **Ажилтны бүртгэлд** (`/admin/directory`) байгаа хүн, эсвэл `ADMIN_EMAILS` |
| Нэр | Асуухгүй — бүртгэлээс автоматаар татна |

Ажилтны бүртгэл хоосон үед зөвхөн `ADMIN_EMAILS` нэвтэрнэ — админ эхлээд
жагсаалтаа импортлох боломжтой байхын тулд.

> ⚠️ Нууц үг нь мэйлээс тааварлагдана. Хэн нэгэн ажилтны мэйлийг мэдвэл түүний
> нэрээр нэвтрэх боломжтой. Энэ бол **SSO хүртэлх түр шийдэл**.

### Ажилтны бүртгэл ([/admin/directory](app/templates/admin_directory.html))

Admin Excel/CSV-ээр `@monos.mn` ажилтнуудын жагсаалтыг оруулна:
`Мэйл · Овог · Нэр · Албан тушаал · Салбар · Хэлтэс · Утас`

Баганын толгойг таньдаг тул **дараалал чөлөөтэй**. Нэвтрэх бүрд овог нэр
(`Б. Болормаа` хэлбэрээр), албан тушаал, салбар нь профайлд автоматаар орно —
ажилтан салбар солиход дараагийн нэвтрэлтэд шинэчлэгдэнэ.

Дахин импорт хийхэд давхардахгүй (мэйлээр upsert), үзэлтийн түүх хөндөгдөхгүй.
